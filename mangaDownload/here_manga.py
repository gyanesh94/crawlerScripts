...
def get_image_links(url):
	...
	div = soup.find("div", {"class": "go_page"})
	span = div.find("span", {"class": "right"})
	options = span.find_all("option")
	pages = []
...
if url:
	src = get_src(url)
	soup = BeautifulSoup(src, 'html.parser')
	ch_div = soup.find('div', {"class" : "detail_list" })
	chapters = get_chapters(ch_div.ul.find_all('a'))
	if chapters:
		i = len(chapters) - 1
